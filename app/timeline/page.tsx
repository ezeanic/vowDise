...
import { Calendar, Clock, AlertTriangle, CheckCircle, Info, ChevronRight } from "lucide-react";
import { Section } from "@/components/ui";
import { bookingTimeline, getTimelineForWeddingDate, getTimelineStatus } from "@/lib/timeline";
...
const statusConfig = {
  overdue: { color: "bg-rose/10 text-rose border-rose/30", icon: AlertTriangle, label: "Overdue" },
  urgent: { color: "bg-amber/10 text-amber border-amber/30", icon: Clock, label: "Book now" },
  "on-track": { color: "bg-sage/10 text-sage border-sage/30", icon: CheckCircle, label: "On track" },
  upcoming: { color: "bg-blue/10 text-blue border-blue/30", icon: Info, label: "Upcoming" },
};
...
function TimelineManager() {
  ...
  const monthsUntil = weddingDate 
    ? Math.max(0, (new Date(weddingDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24 * 30))
    : 0;
  ...
  const sortedTimeline = useMemo(() => {
    return weddingDate 
      ? timeline.sort((a, b) => {
          ...
          const priorityOrder = { overdue: 0, urgent: 1, "on-track": 2, upcoming: 3 };
  ...
  return (
    <main className="min-h-screen bg-[#fbf7ef] text-charcoal">
      <Section className="space-y-8 pb-20 pt-8 sm:pt-10">
        <div>
          <p className="text-sm font-bold uppercase tracking-[0.18em] text-rose">Booking timeline</p>
          <h1 className="mt-2 font-serif text-5xl font-semibold">Know when to book what</h1>
          <p className="mt-4 text-charcoal/68">A timeline of when to book each vendor type to stay on track and avoid stress.</p>
        </div>

        {/* Wedding Date Input */}
        <div className="rounded-xl border border-champagne/30 bg-white p-6 shadow-sm">
          <label className="block text-sm font-semibold text-charcoal/75">Your wedding date</label>
        ...
        <div className="space-y-4">
          {sortedTimeline.map((item) => {
            ...
            const status = weddingDate ? statusConfig[getTimelineStatus(monthsUntil, item.monthsBefore)] : null;

            return (
              <div key={item.category} className="rounded-xl border border-champagne/30 bg-white p-6 shadow-sm">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 flex-wrap">
                      <h3 className="font-semibold text-charcoal">{item.category}</h3>
                    ...
                    <div className="mt-3 flex items-center gap-2 text-sm text-charcoal/60">
                      <Clock size={14} />
                      <span>Book {item.monthsBefore} months before wedding</span>
                    </div>
                  </div>
                ...
                <div className="mt-4 rounded-lg bg-ivory p-4">
                  <p className="text-sm font-semibold text-charcoal/70">Pro tips:</p>
        ...
        {!weddingDate && (
          <div className="rounded-xl border border-champagne/30 bg-white p-8 text-center">
            ...
            <p className="mt-4 font-semibold text-charcoal">Set your wedding date</p>
            <p className="mt-2 text-charcoal/60">Enter your wedding date above to see personalized booking deadlines.</p>
          </div>
        )}
      </Section>
    </main>
