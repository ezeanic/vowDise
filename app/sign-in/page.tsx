...
import { friendlyAuthError, signInWithGoogle, signInWithPassword, type UserCapability } from "@/lib/account-service";
...
export default function SignInPage() {
  ...
  const destination = (() => {
    if (typeof window === "undefined") return capability === "vendor" ? "/vendor-onboarding" : "/budget";
      ...
      <Section className="min-h-[calc(100vh-4rem)] content-center">
        <div className="mx-auto grid max-w-5xl gap-6 lg:grid-cols-[0.85fr_1.15fr]">
          <div className="rounded-[8px] bg-charcoal p-8 text-white shadow-soft">
            ...
            <h1 className="mt-5 font-serif text-5xl font-semibold">Sign in to Vowdise</h1>
            <p className="mt-4 leading-7 text-white/68">
              Access your saved vendors, budget, quote requests, and planning tools before starting another setup flow.
            </p>
          </div>
          ...
          <div className="rounded-[8px] border border-champagne/55 bg-white p-6 shadow-soft sm:p-8">
            <form className="grid gap-4" onSubmit={handlePasswordSignIn}>
              ...
              <input required name="password" type="password" placeholder="Password" className="rounded-[8px] border border-champagne bg-ivory px-4 py-3 outline-none focus:ring-2 focus:ring-gold/30" />
            ...
            <p className="mt-5 text-center text-sm text-charcoal/55">
              New here?{" "}
              <Link href={`/create-account?capability=${capability}`} className="font-bold text-rose">
                Create an account
              </Link>
            </p>
          </div>
