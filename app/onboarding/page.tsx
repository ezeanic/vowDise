...
import { getStoredAccount, saveBudget, saveCoupleProfile } from "@/lib/account-service";
import { citiesForState, formatLocation, locationStates } from "@/lib/location";

const steps = [
  { key: "location", label: "Wedding location", type: "text", placeholder: "San Jose, CA" },
  { key: "date", label: "Wedding date (optional)", type: "date", optional: true },
  { key: "budget", label: "Total budget", type: "number", placeholder: "25000" },
...
export default function OnboardingPage() {
  ...
  function updateLocation(nextValues: Record<string, string>) {
    ...
    <main>
      <Section className="min-h-[calc(100vh-4rem)] content-center">
        <div className="mx-auto max-w-2xl rounded-[8px] border border-champagne/55 bg-white p-6 shadow-soft sm:p-8">
          <p className="text-sm font-bold uppercase tracking-[0.18em] text-rose">Step {step + 1} of {steps.length}</p>
          <h1 className="mt-3 font-serif text-4xl font-semibold">Build your wedding plan</h1>
          ...
          <form
            ...
            onSubmit={async (event) => {
              ...
              if (step < steps.length - 1) setStep(step + 1);
                ...
                if (budgetValue > 0) {
                  await saveBudget(account.uid, { total: budgetValue, items: buildBudget(budgetValue) });
                }
                router.push("/budget");
              }
            }}
          >
            ...
            {current.key === "location" ? (
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                <select
                  ...
                  className="w-full rounded-[8px] border border-champagne bg-ivory px-4 py-4 text-lg outline-none focus:ring-2 focus:ring-gold/30"
                >
                ...
                <div>
                  <input
                    ...
                    className="w-full rounded-[8px] border border-champagne bg-ivory px-4 py-4 text-lg outline-none focus:ring-2 focus:ring-gold/30 disabled:cursor-not-allowed disabled:opacity-60"
                  />
              ...
              <input
                ...
                onChange={(event) => setValues({ ...values, [current.key]: event.target.value })}
            ...
            <div className="mt-6 flex justify-between">
              ...
              <Button type="submit">{step === steps.length - 1 ? "Create Plan" : "Continue"} <ArrowRight size={18} /></Button>
            </div>
          </form>
        </div>
      </Section>
