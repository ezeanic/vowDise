...
export default function ForVendorsPage() {
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "vowdise-vendor-profile-changed",
      syncVendorProfile,
    );
  ...
  return (
    <main className="bg-ivory text-charcoal">
      <section className="via-rose-950 relative overflow-hidden bg-gradient-to-br from-slate-950 to-charcoal text-white">
        ...
        <div className="relative mx-auto grid min-h-[calc(100vh-4rem)] max-w-7xl content-center px-4 py-16 sm:px-6 lg:px-8">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-8 max-w-3xl font-serif text-5xl font-semibold leading-tight sm:text-6xl">
              Elegant vendor profiles for every business you own.
            </h1>
            <p className="mt-6 max-w-2xl text-lg leading-8 text-white/70">
              Create and manage multiple businesses in one account, keep your
              public profile polished, and open the right dashboard for each
              brand.
            </p>
          ...
          <div className="mx-auto max-w-6xl rounded-[32px] border border-champagne/20 bg-white/95 p-8 shadow-[0_30px_80px_-40px_rgba(15,23,42,0.12)] backdrop-blur-sm">
            <div className="mb-10 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
              <div className="max-w-xl">
                ...
                <h2 className="mt-3 font-serif text-4xl font-semibold">
                  Select the business to manage
                </h2>
                <p className="mt-3 text-base text-charcoal/70">
                  Launch the dashboard for any business, or add another one to
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {vendorProfiles.map((profile) => (
                ...
                >
                  <div className="mb-5 flex items-center justify-between gap-3">
                    <div>
                      <p className="text-sm font-semibold uppercase tracking-[0.2em] text-rose">
                        {[
                          profile.category || "Untitled category",
                          profile.category === "Venues"
                            ? profile.venueSubcategory
                            : null,
                        ]
                          .filter(Boolean)
                          .join(" / ")}
                      ...
                      <p className="mt-3 text-2xl font-semibold text-charcoal">
                        {profile.businessName || "Untitled business"}
                      </p>
                    </div>
                  ...
                  <div className="space-y-3 text-sm text-charcoal/70">
                    ...
                    <p>
                      From $
                      {Number(profile.startingPrice || 0).toLocaleString()}
                    </p>
                  </div>
                  <button
                    type="button"
                    ...
                    onClick={(event) => {
                      ...
                      router.push(
                        `/messages?vendorProfile=${encodeURIComponent(profile.id)}`,
                      );
                    }}
                  >
                    Open messages
              ...
              <div
                ...
                onClick={() =>
                  router.push(
                    localStorage.getItem("vowdiseAccount")
                      ? "/vendor-onboarding"
              ...
              >
                ...
                <p className="mt-2 text-sm text-charcoal/60">
                  Create a new business profile in your account.
                </p>
              </div>
            </div>
          </div>
